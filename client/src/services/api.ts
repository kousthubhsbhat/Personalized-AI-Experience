...
// Default API Base: use proxy in dev, or local storage override if set
const isLocal = typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');
const API_BASE_URL = isLocal ? (import.meta.env.VITE_API_BASE_URL || '/api') : '/api';
...
// Optional direct Supabase client for client-side fallback
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || 'https://qhjtdhzasrqjvwbrzqqx.supabase.co';
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || 'mock-anon-key';
export const supabase = (supabaseUrl && !supabaseUrl.includes('mock'))
  ? createClient(supabaseUrl, supabaseAnonKey)
  : null;

// ==========================================
// In-Browser Resilient State (For Vercel Standalone)
// ==========================================
...
export const api = {
  ...
  checkHealth: async () => {
    try {
      ...
      return { status: 'ok', service: 'SkillPulse Cloud Client Engine', model: 'gemini-2.5-flash' };
    }
  },
  ...
  getCurrentPathway: async (): Promise<{ pathway: LearningPathway; skills: UserSkill[]; profile: Profile }> => {
    try {
      ...
      if (res.data && res.data.pathway) {
        return res.data;
      }
      if (res.data && res.data.title) {
        return { pathway: res.data, skills: localSkills, profile: localProfile };
      }
    } catch {
      // Fallback
    }
  ...
  submitQuiz: async (
    ...
    try {
      ...
      // Offline / Vercel standalone evaluation
