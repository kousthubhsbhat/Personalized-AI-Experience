...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full border-b border-slate-800/80 bg-[#0B0F19]/85 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link to={isAuthenticated ? "/pathway" : "/login"} className="flex items-center gap-3 group">
            ...
            {/* Main Navigation Links - Only enabled when authenticated */}
            {isAuthenticated && (
          ...
          {/* Right Action Tools & Profile / Sign In */}
          <div className="flex items-center gap-2.5">
            {!isAuthenticated ? (
              // Unauthenticated View: Clear Sign In / Register Actions
              <div className="flex items-center gap-2">
                <button
                  onClick={() => openAuthModal('signin')}
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-medium transition-all"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 hover:opacity-95 text-white text-xs font-semibold shadow-glow transition-all"
                >
              ...
              // Authenticated View: Active options and User Profile
              <>
                    ...
                    <div className="absolute right-0 mt-2 w-72 bg-[#0E1322] border border-slate-700 rounded-2xl shadow-2xl p-3 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-3">
                      ...
                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-white text-sm">{profile?.full_name || 'Learner'}</span>
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                            @{profile?.username || 'user'}
                          </span>
                        </div>
                        <p className="text-[11px] text-cyan-400 font-medium truncate">{profile?.target_role || 'Learner Track'}</p>
                        <div className="flex items-center gap-1.5 text-[10px] text-emerald-400 font-mono pt-0.5">
                          <ShieldCheck className="w-3 h-3" />
                          <span>Active Encrypted Profile</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 text-xs">
                        ...
                        <div className="pt-1 border-t border-slate-800">
                          <button
                            ...
                            className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-rose-400 hover:text-rose-300 hover:bg-rose-950/30 transition-colors text-left font-medium"
                          >
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-bold text-white text-xs">{profile?.full_name || 'Learner'}</p>
                      <p className="text-[10px] text-cyan-400">@{profile?.username || 'user'}</p>
                    </div>
                  </div>
                  <button
                    onClick={handleSignOut}
                    className="px-2.5 py-1 rounded-lg bg-rose-900/50 border border-rose-700/50 text-[10px] font-semibold text-rose-300"
                  >
                    Sign Out
                  </button>
                </div>
              ...
              <div className="p-3 space-y-2">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-slate-800 text-xs font-semibold text-white flex items-center justify-center gap-2"
                >
                  <LogIn className="w-4 h-4 text-cyan-400" />
                  Sign In
                </button>
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 text-xs font-semibold text-white flex items-center justify-center gap-2 shadow-glow"
                >
      ...
      {isAuthenticated && isCertOpen && <CertificateModal onClose={() => setIsCertOpen(false)} />}
      {isAuthenticated && isSettingsOpen && <SettingsModal onClose={() => setIsSettingsOpen(false)} />}
      {isAuthenticated && isSandboxOpen && <CodeSandboxModal onClose={() => setIsSandboxOpen(false)} />}
    </>
  );
};
